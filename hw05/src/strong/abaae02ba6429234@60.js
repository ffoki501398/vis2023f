...
function _simple1(output,d3,drag,url_list,invalidation)
{
  // 指定圖表的尺寸。
  const width = 1000;
  const height = 400;
  ...
  // 調整作業成績
  const simple = { ...output };
  simple.children = [output.children[6]];
  let score_2 = [10, 10, 8, 8, 10, 8, 10, 8]
  for (let i = 0; i < 8; i++) {
...
function _simple2(d3,output,drag,url_list,invalidation)
{
  // 指定圖表的尺寸。
  const width = 1500;
  const height = 1500;
...
export default function define(runtime, observer) {
  ...
  main.variable(observer("simple1")).define("simple1", ["output","d3","drag","url_list","invalidation"], _simple1);
  main.variable(observer()).define(["md"], _7);
  main.variable(observer("simple2")).define("simple2", ["d3","output","drag","url_list","invalidation"], _simple2);
  return main;
}
